...
ros::Publisher esvio_pose;
ros::Publisher gt_pose;
ros::Publisher pub_latest_esvio;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    esvio_pose=n.advertise<geometry_msgs::PoseStamped>("esvio_pose", 1000);
    gt_pose=n.advertise<geometry_msgs::PoseStamped>("gt_pose", 1000);
    pub_latest_esvio = n.advertise<geometry_msgs::PoseStamped>("imu_evio", 1000);
...
void pubGT(const geometry_msgs::PoseStamped::ConstPtr& msg){
...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header)
{
    ...
    pub_latest_esvio.publish(pose_stamped);//pub odom as geometry_msgs::PoseStamped
}
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        esvio_pose.publish(pose_stamped);
        ...
        ofstream foutC(ESVIO_RESULT_PATH, ios::app);
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    for (auto &it_per_id : estimator.f_manager.stereo_feature)
    {
        int used_num;
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // // camera_right frame
    // transform.setOrigin(tf::Vector3(estimator.tic[2].x(),
    //                                 estimator.tic[2].y(),
    //                                 estimator.tic[2].z()));
    // q.setW(Quaterniond(estimator.ric[2]).w());
    // q.setX(Quaterniond(estimator.ric[2]).x());
    // q.setY(Quaterniond(estimator.ric[2]).y());
    // q.setZ(Quaterniond(estimator.ric[2]).z());
    // transform.setRotation(q);
    // br.sendTransform(tf::StampedTransform(transform, header.stamp, "body", "camera_right"));

    // event frame
    transform.setOrigin(tf::Vector3(estimator.tic[1].x(),
                                    estimator.tic[1].y(),
                                    estimator.tic[1].z()));
    ...
    transform.setRotation(q);
    br.sendTransform(tf::StampedTransform(transform, header.stamp, "body", "event"));

    // // event_right frame
    // transform.setOrigin(tf::Vector3(estimator.tic[3].x(),
    //                                 estimator.tic[3].y(),
    //                                 estimator.tic[3].z()));
    // q.setW(Quaterniond(estimator.ric[3]).w());
    // q.setX(Quaterniond(estimator.ric[3]).x());
    // q.setY(Quaterniond(estimator.ric[3]).y());
    // q.setZ(Quaterniond(estimator.ric[3]).z());
    // transform.setRotation(q);
    // br.sendTransform(tf::StampedTransform(transform, header.stamp, "body", "event_right"));
...
void pubKeyframe(const Estimator &estimator)//pub keyframe pose and map point
{
