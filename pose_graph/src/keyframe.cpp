...
/**
 * @brief Construct a new Key Frame:: Key Frame object. Calculate the descriptor of the existing feature points, and extract the fast corner points and calculate the descriptor 
 * 
 * @param[in] _time_stamp keyframe timestamp
 * @param[in] _index KF index
 * @param[in] _vio_T_w_i vio pose
 * @param[in] _vio_R_w_i 
 * @param[in] _image corresponding image
 * @param[in] _point_3d 3D point in the world frame, keyframe
 * @param[in] _point_2d_uv pixel coordinate
 * @param[in] _point_2d_norm normalized camera coordinate
 * @param[in] _point_id map point id
 * @param[in] _sequence 
 */
...
/**
 * @brief caculate the descriptor of the existing feature points
 * compute similarity to check whether other keyframes form a loop with current keyframe
 * if the similarity score reaches the set threshold, it can be considered that the frame is the loop closure frame.
 */
...
void KeyFrame::savecomputeWindowBRIEFPoint()
{
	...
	for(int i=0;i< (int) window_keypoints.size();i++){
		keypoints.push_back(window_keypoints[i]);
		brief_descriptors.push_back(window_brief_descriptors[i]);
                            ...
                            cv::Point2f &best_match_norm)
{
    ...
    for(int i = 0; i < (int)descriptors_old.size(); i++)
    {
        ...
        int dis = HammingDis(window_descriptor, descriptors_old[i]);// calculate the hanmming scores between two descriptors
        if(dis < bestDist)
        {
            bestDist = dis;
            bestIndex = i;
    ...
    if (bestIndex != -1 && bestDist < 80)// if the best distance is less than 80, it is considered as a successful match
    {
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	TicToc t_match;
		...
		if (SAVE_LOOP_MATCH)    
	    {
	        ...
	        ostringstream path;
	        path << "/home/cpy/catkin_ws_dvs/src/EVIO/pose_graph/loop_image/"
	                << index << "-"
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
				...
				putText(notation, "current frame: " + to_string(index), cv::Point2f(20, 30), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);
	            ...
	            if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	            {
					...
					ostringstream path;
					path <<  "../../esvio_result_visualization/loop_image/"
							<<"cur_frame:"<< index << "---"
